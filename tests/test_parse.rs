...
fn should_be_case_insensitive() {
    assert_eq!(parse("53 YeArS").unwrap(), 1672552800000);
    assert_eq!(parse("53 WeEkS").unwrap(), 32054400000);
    assert_eq!(parse("53 DaYS").unwrap(), 4579200000);
    assert_eq!(parse("53 HoUrs").unwrap(), 190800000);
    assert_eq!(parse("53 MiLliSeCondS").unwrap(), 53);
}

#[test]
fn should_work_with_numbers_starting_with_dot() {
    assert_eq!(parse(".5ms").unwrap(), 1); // 0.5 rounds to 1
}
...
fn long_should_not_throw_error() {
    assert!(parse("53 milliseconds").is_ok());
}
...
fn long_should_convert_months() {
    assert_eq!(parse("1 month").unwrap(), 2629800000);
}
...
fn should_throw_error_on_long_string() {
    ...
    assert!(parse(&long_string).is_err());
}
